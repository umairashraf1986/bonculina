 ...
 */
class Refunds implements Cashier_Process_Interface {
	 ...
	 * @throws Invalid_Argument
	 */
	public function refund( $transaction_id, $arguments = array() ) {
		$this->set_transaction_id( $transaction_id );
		$url = $this->get_resource_endpoint() . $this->service->get_action_endpoint( 'refund' );

		if ( ! is_array( $arguments ) ) {
			$arguments = array( 'transaction' => array() );
		}

		$refund = $this->service->get_client()->send_request( $url, $arguments );

		$this->service->get_client()->check_response_code( $refund, 'cashier_refund' );

		if ( empty( $refund ) ) {
			throw new Exception( __( 'There was a problem with refund request. Pay360 response was empty.', \WC_Pay360::TEXT_DOMAIN ) );
		}
