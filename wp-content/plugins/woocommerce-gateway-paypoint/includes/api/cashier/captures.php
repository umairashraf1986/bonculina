 ...
 */
class Captures implements Cashier_Process_Interface {
	 ...
	 * @throws Invalid_Argument
	 */
	public function capture( $transaction_id, $arguments ) {
		$this->set_transaction_id( $transaction_id );
		$url = $this->get_resource_endpoint() . $this->service->get_action_endpoint( 'capture' );

		if ( ! is_array( $arguments ) ) {
			$arguments = array();
		}

		$capture = $this->service->get_client()->send_request( $url, $arguments );

		$this->service->get_client()->check_response_code( $capture, 'cashier_capture' );

		if ( empty( $capture ) ) {
			throw new Exception( __( 'There was a problem with capture request. Pay360 response was empty.', \WC_Pay360::TEXT_DOMAIN ) );
		}
