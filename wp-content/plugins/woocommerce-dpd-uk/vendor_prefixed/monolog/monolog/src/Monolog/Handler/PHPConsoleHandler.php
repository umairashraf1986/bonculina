...
use DpdUKVendor\PhpConsole\Connector;
use DpdUKVendor\PhpConsole\Handler;
use DpdUKVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \DpdUKVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \DpdUKVendor\PhpConsole\Connector $connector = null, $level = \DpdUKVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('DpdUKVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\DpdUKVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \DpdUKVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \DpdUKVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\DpdUKVendor\PhpConsole\Helper::isRegistered()) {
            \DpdUKVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \DpdUKVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \DpdUKVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \DpdUKVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \DpdUKVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
