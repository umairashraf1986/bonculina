...
use DpdUKVendor\Monolog\Handler\AbstractHandler;
/**
...
class ErrorHandler
{
    ...
    public static function register(\Psr\Log\LoggerInterface $logger, $errorLevelMap = array(), $exceptionLevel = null, $fatalLevel = null)
    {
        ...
        \class_exists('DpdUKVendor\\Psr\\Log\\LogLevel', \true);
    ...
    {
        $this->logger->log($this->uncaughtExceptionLevel === null ? \Psr\Log\LogLevel::ERROR : $this->uncaughtExceptionLevel, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \DpdUKVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), array('exception' => $e));
    ...
    public function handleFatalError()
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \DpdUKVendor\Monolog\Logger) {
                foreach ($this->logger->getHandlers() as $handler) {
                    if ($handler instanceof \DpdUKVendor\Monolog\Handler\AbstractHandler) {
                        $handler->close();
                    }
                }
            }
