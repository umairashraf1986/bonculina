...
use DpdUKVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use DpdUKVendor\WPDesk\Logger\WC\WooCommerceCapture;
use DpdUKVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \DpdUKVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\DpdUKVendor\Monolog\Registry::hasLogger($name)) {
            \DpdUKVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\DpdUKVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \DpdUKVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \DpdUKVendor\Monolog\Logger($name);
        }
        if (\DpdUKVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \DpdUKVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\DpdUKVendor\Monolog\Logger $logger)
    {
        if (!\defined('DpdUKVendor\\WC_LOG_THRESHOLD')) {
            \define('DpdUKVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \DpdUKVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \DpdUKVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \DpdUKVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \DpdUKVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\DpdUKVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \DpdUKVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \DpdUKVendor\Monolog\Registry::hasLogger($name);
    }
}
