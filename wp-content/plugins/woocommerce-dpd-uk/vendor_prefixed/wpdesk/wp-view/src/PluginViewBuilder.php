...
namespace DpdUKVendor\WPDesk\View;

use DpdUKVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use DpdUKVendor\WPDesk\View\Resolver\ChainResolver;
use DpdUKVendor\WPDesk\View\Resolver\DirResolver;
use DpdUKVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    public function createSimpleRenderer()
    {
        ...
        $resolver->appendResolver(new \DpdUKVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \DpdUKVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \DpdUKVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
