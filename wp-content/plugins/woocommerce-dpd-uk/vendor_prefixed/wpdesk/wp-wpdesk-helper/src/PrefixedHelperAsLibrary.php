...
namespace DpdUKVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use DpdUKVendor\WPDesk\Helper\Debug\LibraryDebug;
use DpdUKVendor\WPDesk\Helper\Integration\LicenseIntegration;
use DpdUKVendor\WPDesk\Helper\Integration\LogsIntegration;
use DpdUKVendor\WPDesk\Helper\Integration\SettingsIntegration;
use DpdUKVendor\WPDesk\Helper\Integration\TrackerIntegration;
use DpdUKVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use DpdUKVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \DpdUKVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \DpdUKVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \DpdUKVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \DpdUKVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \DpdUKVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \DpdUKVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \DpdUKVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \DpdUKVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \DpdUKVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \DpdUKVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \DpdUKVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
