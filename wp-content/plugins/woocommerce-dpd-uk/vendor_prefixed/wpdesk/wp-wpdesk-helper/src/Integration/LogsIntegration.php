...
use DpdUKVendor\WPDesk\Logger\WPDeskLoggerFactory;
 ...
 */
class LogsIntegration implements \DpdUKVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    /** @var Logger */
    private $logger;
    public function __construct(\DpdUKVendor\WPDesk\Helper\Page\SettingsPage $settings_page)
    {
    ...
    {
        $logger_factory = new \DpdUKVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        if (\property_exists($logger_factory, 'shouldLoggerBeActivated')) {
            ...
            $this->logger = new \DpdUKVendor\Monolog\Logger('fallback');
        }
    }
    private function maybe_show_logging_notice()
    {
        ...
        if ($debug_log_enabled) {
            if (\apply_filters('wpdesk_helper_show_log_notices_library', \true)) {
                new \DpdUKVendor\WPDesk\Notice\Notice(\sprintf(
                    // Translators: link.
                    ...
                    '</a>'
                ), \DpdUKVendor\WPDesk\Notice\Notice::NOTICE_TYPE_INFO);
            }
        }
    }
